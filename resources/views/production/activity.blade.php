...
<div class="page">
    <div class="nav-row">
        <a class="back" href="{{ route('apt.management') }}">← Volver a inicio de turno</a>
        <a class="back" href="{{ route('apt.production') }}">← Volver a Gestión de almacenes</a>
    </div>
    <div class="shell">
    <header class="hero">
    ...
    <main class="panel" data-shift-id="{{ $registration->id ?? '' }}">
        <section class="card">
            ...
            <label class="field">Usuario asignado<input readonly value="{{ optional(optional($registration)->user)->name }}"></label>
            <label class="field">Puesto<input readonly value="{{ optional($registration)->position }}"></label>
            <label class="field">Turno<input readonly value="{{ optional($registration)->shift }}"></label>
            <label class="field">Fecha<input readonly value="{{ optional(optional($registration)->started_at)->format('d/m/Y') }}"></label>
        </section>
        <section class="card">
            <div class="card-head">
                <h2>Información de lote de producción en proceso</h2>
                <p>{{ $registration ? 'Identificación del lote activo en recepción.' : 'El lote permanece vacío hasta que el Jefe de Almacén te asigne uno.' }}</p>
            </div>
            <label class="field wide">No. de lote<input readonly value="{{ optional(optional($registration)->lot)->folio }}"></label>
            <label class="field">Producto<input readonly value="{{ str_contains(strtoupper((string) optional(optional($registration)->lot)->plant_origin), 'UREA') ? 'UREA AGRICOLA' : (optional(optional($registration)->lot)->folio ? 'Automático' : '') }}"></label>
            <label class="field">Origen<input readonly value="{{ optional(optional($registration)->lot)->plant_origin }}"></label>
            <label class="field">Disposición<input readonly value="{{ optional(optional($registration)->lot)->warehouse }}"></label>
        ...
        <section class="card history">
            ...
            <div class="history-filters">
                ...
                <label>Fecha<input type="date" data-history-date value="{{ $activityDate ?? '' }}"></label>
            ...
            <table>
                ...
                <tbody>
                    @forelse($activities as $activity)
                        <tr>
                            <td>{{ $activity->occurred_at->format('d/m/Y') }}</td>
                            <td>{{ $activity->occurred_at->format('H:i:s') }}</td>
                            <td>{{ optional(optional($registration)->lot)->folio }}</td>
                            <td>{{ optional(optional($registration)->lot)->warehouse }}</td>
                            <td>{{ $activity->location ?: 'Automático' }}</td>
                            <td>{{ str_contains(strtoupper((string) optional(optional($registration)->lot)->plant_origin), 'UREA') ? 'UREA AGRICOLA' : (optional(optional($registration)->lot)->folio ? 'Automático' : '') }}</td>
